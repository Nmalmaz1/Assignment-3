...
class AdventureLandApp:
    ...
    def purchase_tickets(self):
        ...
        self.data["tickets"] = self.data_layer.load_tickets()  # Reload tickets to include admin changes
        ticket_details = [{"ticket_type": ticket.get_ticket_type(), "description": ticket.get_description(), "price": ticket.get_price(), "validity": ticket.get_validity(), "limitations": ticket.get_limitations(), "discount": ticket.get_discount()} for ticket in self.data["tickets"]]
        # Title
        tk.Label(self.root, text="Purchase Tickets", font=("Arial", 18, "bold")).grid(row=0, column=0, columnspan=7, pady=20)
        # Column headers
        headers = ["Ticket Type", "Description", "Price (AED)", "Validity", "Discount", "Limitations", "Action"]
        col_index = 0  # Manual column index
        for header in headers:
            tk.Label(self.root, text=header, font=("Arial", 12, "bold"), anchor="center").grid(row=1, column=col_index, padx=10, pady=5)
            col_index += 1

        # Display updated ticket details
        row_index = 2  # Manual row index
        for ticket in ticket_details:
            discounted_price = ticket["price"] * (1 - ticket["discount"] / 100)
            details = [ticket["ticket_type"], ticket["description"], f"{discounted_price:.2f}", ticket["validity"],
                       f"{ticket['discount']}%", ticket["limitations"]]
            col_index = 0  # Reset column index for each row
            for detail in details:
                tk.Label(self.root, text=detail, font=("Arial", 10), wraplength=150, justify="left").grid(row=row_index,
                                                                                                          column=col_index,
                                                                                                          padx=5,
                                                                                                          pady=5)
                col_index += 1
            # Add 'Add to Cart' button
            tk.Button(self.root, text="Add to Cart", font=("Arial", 10),
                      command=lambda t=ticket: self.add_to_cart(t)).grid(row=row_index, column=col_index, padx=10,
                                                                         pady=5)
            row_index += 1

        # Back Button
        tk.Button(self.root, text="Back to Home", command=lambda: self.show_home_page("customer"), font=("Arial", 14),
                  width=15).grid(row=row_index + 2, column=3, pady=20)
    def create_customer_account_settings(self):
        self.root.geometry("180x200")  # Adjust window size to fit ticket details
        # Clear the window
        for widget in self.root.winfo_children():
            widget.destroy()
    ...
    def view_cart(self):
        self.root.geometry("380x500")  # Adjust window size for cart details
        # Clear the current window
        for widget in self.root.winfo_children():
            widget.destroy()
        # Title
